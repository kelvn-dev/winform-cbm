...
namespace CBM.Presenters {
  public class VehicleTypePresenter {

    private VehicleTypeForm view;
    private BindingSource bindingSource;

    public VehicleTypePresenter(VehicleTypeForm view) {
      this.view = view;
    ...
    private void Save(object sender, EventArgs e) {
      ...
      try {
        //ModelValidation.Validate(vehicleType);
        if (view.isUpdate) {
          vehicleType.id = view.id;
    ...
    private void LoadListingData() {
      List<VehicleType> vehicleTypeList = VehicleTypeService.GetPaginatedData(
      ...
      bindingSource.DataSource = vehicleTypeList;
    }

    private void UpdatePageLabel() {
      view.totalPages = VehicleTypeService.GetTotalPages();
    ...
    private void LoadSelectedData(object sender, EventArgs e) {
      view.isUpdate = true;
      VehicleType vehicleType = bindingSource.Current as VehicleType;
      view.id = vehicleType.id;
      view.name = vehicleType.name.Trim();
      view.description = vehicleType.description.Trim();
      if (vehicleType.logo != null) {
        MemoryStream stream = new MemoryStream(vehicleType.logo);
    ...
    private void DeleteSelectedData(object sender, EventArgs e) {
      VehicleType vehicleType = bindingSource.Current as VehicleType;
      VehicleTypeService.DeleteById(vehicleType.id);
      LoadListingData();
    }
  }
}
